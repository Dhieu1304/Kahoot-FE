export const SET_PRESENTATION = "SET_PRESENTATION";
export const SET_SLIDES = "SET_SLIDES";
export const CHANGE_SLIDE = "CHANGE_SLIDE";
export const SET_CURRENT_SLIDE = "SET_CURRENT_SLIDE";
export const SET_CURRENT_SLIDE_INDEX = "SET_CURRENT_SLIDE_INDEX";
export const FETCHING_API = "FETCHING_API";
export const FETCHING_API_SUCCESS = "FETCHING_API_SUCCESS";
export const FETCHING_API_FAILED = "FETCHING_API_FAILED";

export const SET_SHOW_SLIDE_LIST = "SET_SHOW_SLIDE_LIST";
export const SET_SHOW_CREATE_NEW_SLIDE_MODAL = "SET_SHOW_CREATE_NEW_SLIDE_MODAL";
export const SET_SHOW_CHANGE_THEME_MODAL = "SET_SHOW_CHANGE_THEME_MODAL";
